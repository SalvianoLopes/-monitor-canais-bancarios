...
### Auditoria preventiva de RDR e PROCON — CONCLUÍDA em 30/08/2026 (commit `0c6f15f`)
Comparados os cabeçalhos reais de `Bacen.xls`/`Procon.xls` (formato HTML atual da MOL) contra todos os campos do parser. Achadas 4 lacunas inequívocas (dado existe na planilha, ficava vazio por nome de coluna diferente) — mesma classe de bug já vista em datas e em demandante/cpf do Consumidor:

| Canal | Campo | Alias antigo | Alias novo adicionado | Resultado |
|---|---|---|---|---|
| RDR | `extra1` / `dados_raw['Data da Captura']` | `Data da Captura`, `Disponibilização A` | `Data de cadastro` | 0/138 → 138/138 |
| RDR | `dados_raw['Data de Ciência']` | `Data de Ciência`, `Disponibilização` | `Data de disponibilização` (já usado no campo funcional `abertura` desde `c718b83`, faltava só no `dados_raw`) | 0/138 → 138/138 |
| PROCON | `cpf` (funcional + `dados_raw`) | `CPF` | `Documento reclamante` | 0/590 → 590/590 |
| PROCON | `data_cadastro_cip` (funcional + `dados_raw`) | `Data Cadastro CIP` | `Data Cadastro` (confirmado distinto de `Data de captura` em 432/590 casos — não é duplicata) | 0/590 → 590/590 |
| PROCON | `dados_raw['Reclamado']` | `Reclamado` | `Reclamada` (mesmo conceito, gênero gramatical diferente na MOL atual) — usado de fato na interface (`_reclamado` na tabela) | 0/590 → 590/590 |

**Explicitamente NÃO alterado** (fora de escopo ou sem correspondência inequívoca):
- `data_ref` e prioridade de Classificação IA — regras congeladas, intocadas.
- `prazo` do PROCON: `Prazo de resposta CIP` não existe mais no formato atual; `Prazo previsto` seria uma correspondência plausível, mas **`prazo` está congelado por decisão explícita** (ver "Prazo — fora de escopo operacional" acima) — não foi tocado mesmo com o candidato disponível.
- `Protocolo Adm.` / `Prazo proc. adm.` do PROCON: sem substituto no formato atual (só existe o flag booleano `Possui Proc. Adm?`, não o número em si) — permanecem vazios, sem alias inventado.
- `colaborador` do RDR: a coluna `Colaborador responsável` existe no arquivo mas contém só o placeholder `'-` em 100% da amostra — não é problema de nomenclatura, não há dado real pra capturar.

Todos os aliases preservam o nome antigo com prioridade (retrocompatibilidade). Testes: as 5 lacunas foram para 100% de preenchimento sem reduzir nenhum campo que já funcionava; compatibilidade retroativa confirmada por teste; regressão completa (IA 164/164, datas 0 inversões, memória A-F 6/6, E2E RDR 6/6) sem quebras. Simulação de upload (sem gravar): RDR 138/138 normalizadas sem erro, PROCON 590/590 normalizadas sem erro — **ambos aptos para o próximo upload real**. Zero alteração no Supabase (totais e duplicados conferidos idênticos antes/depois do deploy). Deploy: commit `0c6f15f`, `dpl_6VSMvQhChonhXD5t3DYAcic6QJUz`, READY.

**Pendência RDR/PROCON encerrada.** Nenhuma nova pendência de auditoria de cabeçalhos MOL está aberta no momento.
...
**Auditoria preventiva de RDR e PROCON antecipada e concluída (commit `0c6f15f`)** — mesmo dia, pendência que tinha acabado de ser registrada foi antecipada a pedido do usuário. Comparados os cabeçalhos reais dos arquivos MOL atuais (`Bacen.xls`, `Procon.xls`) contra o parser: achadas as mesmas 4 lacunas de nomenclatura já vistas em datas/Consumidor — `extra1`/`dados_raw['Data da Captura']` do RDR (alias `Data de cadastro`), `dados_raw['Data de Ciência']` do RDR (faltava a mesma prioridade a `Data de disponibilização` que o campo funcional já tinha), `cpf` do PROCON (alias `Documento reclamante`, 590/590 no formato CPF válido), `data_cadastro_cip` do PROCON (alias `Data Cadastro`, confirmado distinto de `Data de captura` em 432/590 casos) e `dados_raw['Reclamado']` do PROCON (alias `Reclamada`, campo usado de fato na interface). Todas as 5 lacunas foram de 0% pra 100% de preenchimento nos arquivos reais, sem regressão em nada que já funcionava. Deliberadamente não tocado: `prazo` do PROCON (congelado, mesmo com candidato plausível `Prazo previsto` disponível), `Protocolo Adm.`/`Prazo proc. adm.` (sem substituto no formato atual), `colaborador` do RDR (coluna existe mas só tem placeholder vazio na fonte). Ver detalhes completos na seção "Estado técnico consolidado" acima.

**Estado ao encerrar a sessão de 30/08/2026:** produção estável em `0c6f15f`; datas corrigidas (fluxo principal do código + 2 backfills); Classificação IA validada (164/164); demandante/CPF do Consumidor corrigidos (campo funcional + fix preventivo em `dados_raw`); RDR/PROCON com aliases MOL atualizados (preventivo, sem backfill); base = RDR 1.459 + Consumidor 2.689 + PROCON 3.851 = **7.999**, duplicados = 0; prazo permanece fora de escopo operacional (decisão reafirmada); nenhuma pendência de auditoria de cabeçalhos MOL em aberto.
...
**Consumidor "27/05 zerado" — não era lacuna, era bug de data (13/08/2026, mesmo dia):** usuário reportou que 27/05/2026 aparecia com 0 registros de Consumidor no painel, mas a planilha do MOL mostrava 17. Usuário subiu `Downloads\MOLReport (5).csv` — planilha do **ano inteiro** de Consumidor (2.471 protocolos, colunas `Tags, Colaborador responsável, Protocolo, Nome fantasia, Situação, Reclamante, CPF, Data de abertura, Capturada em`), pra investigação completa antes de qualquer alteração ("antes de commitar, me envie a análise").
- Carregada em tabela de staging (`stage_consumidor_ano`, criada e dropada na mesma sessão) em 10 lotes de ~250 linhas — mesma técnica de outras rodadas grandes desse dia.
- **Achado 1 — bug de data, não lacuna:** os 17 registros de 27/05 **já existiam no banco**, só que gravados com `abertura`/`data_ref` = **28/05/2026** (1 dia à frente do real). Investigando mais a fundo (comparação `numero` a `numero` contra o arquivo inteiro), achado o mesmo padrão em **85 registros espalhados pelo ano todo** (não só 27/05) — todos com a data do banco exatamente 1 dia à frente da real. Mais **3 registros** com erro de 2-3 dias. Total: **88 registros com `abertura`/`data_ref` errada**, corrigidos via `UPDATE` (backup prévio em `canais_criticos_demandas_backup_20260813_consumidor_data_errada`, 88 linhas) usando a coluna "Capturada em" (com hora) do arquivo pra `abertura` e "Data de abertura" pra `data_ref`.
- **Achado 2 — 25 registros genuinamente novos:** não relacionados ao problema original — são só os protocolos de 12 e 13/08/2026 (dia de ontem/hoje) que ainda não tinham sido subidos no app. Inseridos com o mesmo formato do `normalizar('consumidor')` do app (tags viram `demanda`, Situação vira `status`, Colaborador responsável vira `colaborador`, Nome fantasia vira `extra2`). Registrado em `canais_criticos_uploads` (usuario="Claude (script)").
...
- **Bug próprio corrigido:** os 25 registros novos inseridos acima saíram sem o campo `prazo` preenchido (esquecimento no script de insert) — como a coluna da tabela rotulada "Data abertura" no app na verdade lê o campo `prazo` pro Consumidor (mapeamento herdado, ver `headsMap.consumidor`/`fieldsMap.consumidor` em `index.html`), esses 25 registros apareciam com "Data abertura" vazia. Corrigido com `UPDATE ... SET prazo = to_char(data_ref,'DD/MM/YYYY')` nos 25.

**RDR/BACEN também sem 12 e 13/08/2026 (13/08/2026, mesmo dia)** — usuário notou que só Consumidor e PROCON tinham dado desses 2 dias, RDR estava zerado. `MOLReport (4).csv` (já no Downloads, usado antes pra backfill de Data da Captura) tinha só **7 protocolos** pra esses 2 dias (4 do dia 12, 3 do dia 13) — bem menos que Consumidor (25) e PROCON (50) no mesmo período, todos com status idêntico "Pendente: IF/AC". **Alertado ao usuário que isso parece um recorte parcial (só pendentes), não a exportação completa do dia** — usuário decidiu inserir mesmo assim ("melhor ter parte do que nada"). Inseridos os 7, sem CPF (esse formato de MOLReport do BACEN não traz essa coluna, mesma limitação já documentada na rodada de recuperação de Data da Captura). Registrado em `canais_criticos_uploads`.
- **Pendência:** RDR de 12-13/08/2026 provavelmente está incompleto (só os 7 "Pendente" capturados, pode ter outros com status diferente que essa planilha específica não trouxe). Se aparecer um export mais completo desses dias, vale conferir de novo.
...
- Os poucos registros que sobraram sem Data da Captura/CPF (~50-114, variando por campo) — **nota corrigida em 13/08/2026:** a explicação original aqui ("são só protocolos recentes de agosto, fora do período") estava errada. Investigado a pedido do usuário (viu no painel um dia — 30/04/2026 — com 6 registros de RDR sem Data da Captura e perguntou se não tinha na planilha): reabri o `Consolidado Geral.xlsx` do Downloads (mesmo arquivo, mtime 12/08 12:16) e conferi — **os 114 sem captura têm `data_ref` espalhado de 02/01/2026 a 11/08/2026, não só agosto.** Pro caso específico de 30/04: a planilha atual **não tem nenhuma linha com Data de Ciência nesse dia** (pula de 29/04 pra 01/05) nem os 6 protocolos exatos, em lugar nenhum das 1.354 linhas. Não é bug de cruzamento (busca por número exato, sem match) — é a própria planilha que veio incompleta/parcial nessa exportação. **Ação:** usuário vai reexportar um Consolidado mais completo do sistema do BACEN; quando subir, rodar o mesmo cruzamento por `numero` de novo pra tentar recuperar mais desses 114.
- **Segunda rodada (13/08/2026, mesmo dia):** usuário subiu `Downloads\MOLReport (3).csv` — formato novo de export do BACEN (não é o `Consolidado Geral.xlsx`), colunas `Colaborador responsável, Número, Nome, Data de captura, Data de cadastro, Data de disponibilização, Prazo, Situação, Respondido na plataforma` — **não tem coluna de CPF**, então só serve pra recuperar Data da Captura, não CPF. 1.316 protocolos únicos, cobre 01/01 a 01/08/2026. Cruzado contra os 114 sem `extra1`: **51 recuperados** (incluindo os 6 do caso de 30/04 investigado antes), backup em `canais_criticos_demandas_backup_20260813_rdr_captura2` (51 linhas) antes do `UPDATE`. **Restam 63 sem Data da Captura, todos de agosto/2026 (02/08 a 11/08)** — fora do range que esse arquivo cobre (só vai até 01/08). RDR: 114 → 63 sem captura (de 1.379 total).
- **Terceira rodada (13/08/2026, mesmo dia):** usuário subiu `Downloads\MOLReport (4).csv`, mesmo formato, só 70 linhas cobrindo 30/07 a 13/08/2026 (export só dos dias mais recentes). Cruzado contra os 63 que sobraram: **os 63 bateram, 100%.** Backup em `canais_criticos_demandas_backup_20260813_rdr_captura3` (63 linhas) antes do `UPDATE`. **RDR: Data da Captura 100% preenchida (1.379 de 1.379).** CPF continua sem cobertura pra esses 114 (nenhuma das duas planilhas MOLReport tem coluna de CPF, só o `Consolidado Geral.xlsx` original tinha) — se precisar fechar CPF também, teria que ser via um export do BACEN que traga esse campo pros mesmos protocolos.

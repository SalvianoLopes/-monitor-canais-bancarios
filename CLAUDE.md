...
- **REGRA GERAL — Prazo = Data da Captura + 10 dias úteis, em TODOS os canais (decidido 13/08/2026, ainda não implementado):** o usuário decidiu que, quando isso for implementado, o `prazo` de qualquer registro (RDR, PROCON, Consumidor) deve ser recalculado como a **data de captura** (`extra1` no RDR / `abertura` em PROCON e Consumidor, que é a "Capturada em") **+ 10 dias úteis**, **sobrescrevendo** o valor de Prazo que vier pronto na planilha — não é um campo novo separado. Isso é diferente da regra específica do BACEN acima (que usa Data de Ciência); o usuário optou por padronizar usando Data da Captura como base em vez de Data de Ciência, pra todos os canais. **Por enquanto isso é só decisão documentada — não implementar cálculo automático nem rodar backfill até o usuário pedir explicitamente**, dado que a classificação de vencido foi removida de propósito do app (ver "Regra de prazo" abaixo) e o app hoje serve só pra registro de entradas.
